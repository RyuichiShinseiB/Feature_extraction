...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = rearrange(
            out, "b h n d -> b n (h d)", h=self.heads, d=self.dim_head
        )
...
class PatchEmbedding(nn.Module):
    def __init__(self, image_size, patch_size, in_channels, embed_dim):
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            nn.Linear(
                in_features=patch_dim, out_features=embed_dim
            ),  # 埋め込みを行う
        )
    ...
    def forward(self, x):
        ...
        W: 入力画像の幅
        """
        return self.to_patch_embedding(
            x
...
class MAE_Decoder(nn.Module):
    def __init__(
        ...
        self.pos_embedding = torch.nn.Parameter(
            torch.rand(1, num_patches + 1, emb_dim)
        )
...
# モデルを保存しておく
torch.save(model.state_dict(), "tmp/MYmodel/MAE_pretrain_params.pth")
...
model.load_state_dict(
    torch.load("/workdir/tmp/MYmodel/MAE_pretrain_params.pth")
)
...
torch.save(
    ...
    "/workdir/tmp/MYmodel/MAE_classifier_params.pth",
)
